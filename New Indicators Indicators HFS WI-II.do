...
merge 1:1 folio using "$dw1/`country'_PH2W1_CT_Ninos", force 
cap drop _merge 
*cap destring folio, replace force
...
local cuts total male female partnered_f unpartnered_f hh_female hh_male I_hh_female I_hh_male N_hh_female N_hh_male parent no_parent age_18_24 age_25_54 age_55_65 primary secondary terciary
 local variables run_out_food run_out_food_pre_pan /*
				 */ ocupado1 ocupado0 formal0 formal1 ganancia01 perdida01 horas0 horas1/*
				 */ ocupado1_e ocupado0_e activo1_e activo0_e inactivo1_e inactivo0_e formal0_e formal1_e ganancia01_e perdida01_e horas0_e horas1_e/*
				 */ hea2 heal3 hea4 hea5 hea9 hea10 hea6 hea7 hea8 /*
				 */ aumento_v14_05 aumento_v14_06 /*
				 */  t_formal_desocupado t_formal_inactivo t_informal_formal t_informal_desocupado t_informal_inactivo  /*
				 ...
				 */ aumento_dom aumento_childcare aumento_acomp income_red income_not_better
...
local countries 876 
*595 598 758 767 809 876

foreach country of local countries {
*local country 501
use "$dw1/`country'_PH2W1_CT_Casos", replace 
tostring folio, replace force
save "$dw1/`country'_PH2W1_CT_Casos", replace 
}
...
local cuts total partnered_f unpartnered_f hh_female hh_male I_hh_female I_hh_male N_hh_female N_hh_male parent no_parent age_18_24 age_25_54 age_55_65 primary secondary terciary		
 local variables run_out_food run_out_food_pre_pan /*
				...
				*/ hea2 heal3 hea4 hea5 hea9 hea10 hea6 hea7 hea8 /*
				*/ aumento_v14_06 aumento_v14_05 /*
				*/ t_formal_informal t_formal_desocupado t_formal_inactivo t_informal_formal t_informal_desocupado t_informal_inactivo t_fuera_informal t_fuera_formal int_head /*
...
==================================================*/
	...
	*use `tablas'
	...
	local variables  run_out_food run_out_food_pre_pan /*
					...
					*/ female male hh_male hh_female_r hh_male_r reconnected
	...
	foreach wave of local waves {	
		foreach cut of local cuts {
			foreach sexo of local sexos {
				...
				replace ocupado_pre  = Value if  Wave=="`wave'" & Variable=="ocupado0_e" & Cut=="`cut'" & Sex=="`sexo'" 
				replace  ocpuado_post = Value if  Wave=="`wave'" & Variable=="ocupado1_e" & Cut=="`cut'" & Sex=="`sexo'"
				local pre ocupado0_e
				local pos ocupado1_e
				
				mean  ocpuado_post ocupado_pre if Wave=="`wave'" & Cut=="`cut'" & Sex=="`sexo'"
